# Lucid Fusion Labs Root Make File
CMAKE_POLICY(SET CMP0004 OLD)
# MESSAGE(FATAL_ERROR "Debug message")

if(NOT CMAKE_BUILD_TYPE)
    set(CMAKE_BUILD_TYPE "Debug")
endif(NOT CMAKE_BUILD_TYPE)

if(CMAKE_BUILD_TYPE MATCHES Debug)
    set(LFL_DEBUG 1)
endif()

if(LFL_CORE_ONLY)
   set(CORE_SUBDIR .)
else(LFL_CORE_ONLY)
   set(CORE_SUBDIR core)
endif(LFL_CORE_ONLY)

include(ExternalProject)
include(BundleUtilities)
include(${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR}/CMake/Autoconf.cmake)
include(${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR}/CMake/util.cmake)
set(LFL_CORE ${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR})
set(LFL_COREBIN ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR})

if(WIN32)
    link_directories("")
    set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} /SAFESEH:NO")
    FOREACH(flag CMAKE_C_FLAGS_RELEASE CMAKE_C_FLAGS_RELWITHDEBINFO CMAKE_C_FLAGS_DEBUG CMAKE_C_FLAGS_DEBUG_INIT
        CMAKE_CXX_FLAGS_RELEASE CMAKE_CXX_FLAGS_RELWITHDEBINFO CMAKE_CXX_FLAGS_DEBUG  CMAKE_CXX_FLAGS_DEBUG_INIT)
        STRING(REPLACE "/MD"  "/MT"  "${flag}" "${${flag}}")
        STRING(REPLACE "/MDd" "/MTd" "${flag}" "${${flag}}")
        SET("${flag}" "${${flag}} /EHsc")
    ENDFOREACH()
else(WIN32)
    set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} -std=c++11") # -stdlib=libc++")
    set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} -std=c++11 -Wno-deprecated-declarations") # -stdlib=libc++")
    #set(CMAKE_EXE_LINKER_FLAGS "-stdlib=libc++")
    #set(CMAKE_SHARED_LINKER_FLAGS "-stdlib=libc++")
    #set(CMAKE_MODULE_LINKER_FLAGS "-stdlib=libc++")
    if(LFL_IPHONE)
        set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} -stdlib=libc++")
        set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} -stdlib=libc++")
    endif(LFL_IPHONE)
endif(WIN32)
add_definitions(-D__STDC_CONSTANT_MACROS -D__STDC_LIMIT_MACROS)

# args
if(LFL_IPHONESIM)
    set(LFL_IPHONE 1)
    set(IPHONESIM "-Simulator")
endif(LFL_IPHONESIM)

# imports
add_subdirectory(${CORE_SUBDIR}/imports)

# macro includes
if(LFL_PROTOBUF)
    include(${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR}/CMake/FindProtoBuf.cmake)
endif(LFL_PROTOBUF)

if(LFL_FLATBUFFERS)
    set(FLATBUFFERS_INCLUDE_DIR ${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR}/imports/flatbuffers/include)
    set(FLATBUFFERS_FLATC_EXECUTABLE ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/flatbuffers/flatc)
    include(${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR}/imports/flatbuffers/CMake/FindFlatBuffers.cmake)
endif(LFL_FLATBUFFERS)

if(LFL_CAPNPROTO)
    set(CAPNP_LIB_KJ          ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/lib/libkj.a)
    set(CAPNP_LIB_KJ-ASYNC    ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/lib/libkj-async.a)
    set(CAPNP_LIB_CAPNP       ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/lib/libcapnp.a)
    set(CAPNP_LIB_CAPNP-RPC   ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/lib/libcapnp-rpc.a)
    set(CAPNP_EXECUTABLE      ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/bin/capnp)
    set(CAPNPC_CXX_EXECUTABLE ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/bin/capnpc-c++)
    set(CAPNP_INCLUDE_DIRS    ${CMAKE_CURRENT_BINARY_DIR}/${CORE_SUBDIR}/imports/capnproto/include)
    INCLUDE(${CMAKE_CURRENT_SOURCE_DIR}/${CORE_SUBDIR}/imports/capnproto/c++/cmake/FindCapnProto.cmake)
endif(LFL_CAPNPROTO)

# macros
macro(lfl_project _name)
    project(${_name})
    set(LFL_PROJECT ${_name})
    set(LFL_PROJECT_DIR ${CMAKE_CURRENT_SOURCE_DIR})
    set(LFL_PROJECT_BINDIR ${CMAKE_CURRENT_BINARY_DIR})
    set(LFAPP_LIB_TYPE STATIC)

    if(LFL_LFLINPUT)
    if(LFL_IPHONE)
        set(LFL_IPHONEINPUT 1)
    elseif(LFL_ANDROID)
        set(LFL_ANDROIDINPUT 1)
    elseif(WIN32)
        set(LFL_WININPUT 1)
    elseif(CMAKE_SYSTEM_NAME MATCHES "Darwin")
        set(LFL_OSXINPUT 1)
    elseif(CMAKE_SYSTEM_NAME MATCHES "Linux")
        set(LFL_X11INPUT 1)
    endif()
    endif(LFL_LFLINPUT)

    if(LFL_LFLVIDEO)
    if(LFL_IPHONE)
        set(LFL_IPHONEVIDEO 1)
    elseif(LFL_ANDROID)
        set(LFL_ANDROIDVIDEO 1)
    elseif(WIN32)
        set(LFL_WINVIDEO 1)
    elseif(CMAKE_SYSTEM_NAME MATCHES "Darwin")
        set(LFL_OSXVIDEO 1)
    elseif(CMAKE_SYSTEM_NAME MATCHES "Linux")
        set(LFL_X11VIDEO 1)
    endif()
    endif(LFL_LFLVIDEO)

    if(LFL_QT)
        set(LFL_GLEW)
        set(CMAKE_AUTOMOC ON)
        find_package(Qt5OpenGL REQUIRED)
        find_package(Qt5WebKit REQUIRED)
        find_package(Qt5WebKitWidgets REQUIRED)
        foreach(_current ${Qt5WebKitWidgets_COMPILE_DEFINITIONS})
            set(QT_DEF ${QT_DEF} "-D${_current}")
        endforeach()
        foreach(_current ${Qt5WebKit_COMPILE_DEFINITIONS})
            set(QT_DEF ${QT_DEF} "-D${_current}")
        endforeach()
        foreach(_current ${Qt5OpenGL_COMPILE_DEFINITIONS})
            set(QT_DEF ${QT_DEF} "-D${_current}")
        endforeach()
        set(QT_INCLUDE ${Qt5WebKitWidgets_INCLUDE_DIRS} ${Qt5WebKit_INCLUDE_DIRS} ${Qt5OpenGL_INCLUDE_DIRS})
        set(QT_LIB ${Qt5WebKitWidgets_LIBRARIES} ${Qt5WebKit_LIBRARIES} ${Qt5OpenGL_LIBRARIES})
    endif(LFL_QT)
endmacro(lfl_project)

macro(lfl_clear_projects)
    set(LFAPP_DEF)
    set(LFAPP_INCLUDE)
    set(LFAPP_LIB)
    set(LFL_ASSIMP)
    set(LFL_AUDIOUNIT)
    set(LFL_BERKELIUM)
    set(LFL_BOX2D)
    set(LFL_CAMERA)
    set(LFL_CAPNPROTO)
    set(LFL_COMMONCRYPTO)
    set(LFL_FFMPEG)
    set(LFL_FLATBUFFERS)
    set(LFL_FREETYPE)
    set(LFL_GEOIP)
    set(LFL_GIF)
    set(LFL_GLOG)
    set(LFL_GLES2)
    set(LFL_GLEW)
    set(LFL_GLFWVIDEO)
    set(LFL_GLFWINPUT)
    set(LFL_GTEST)
    set(LFL_HARFBUZZ)
    set(LFL_HEADLESS)
    set(LFL_ICONV)
    set(LFL_JPEG)
    set(LFL_JSONCPP)
    set(LFL_JUDY)
    set(LFL_LAME)
    set(LFL_LFLINPUT)
    set(LFL_LFLVIDEO)
    set(LFL_LIBCSS)
    set(LFL_LIBARCHIVE)
    set(LFL_LIBCLANG)
    set(LFL_LUA)
    set(LFL_OPENCV)
    set(LFL_OPENSSL)
    set(LFL_OSXINPUT)
    set(LFL_OSXVIDEO)
    set(LFL_PCAP)
    set(LFL_PNG)
    set(LFL_PROTOBUF)
    set(LFL_PORTAUDIO)
    set(LFL_QT)
    set(LFL_REGEX)
    set(LFL_SDLINPUT)
    set(LFL_SDLVIDEO)
    set(LFL_SREGEX)
    set(LFL_V8JS)
    set(LFL_WININPUT)
    set(LFL_WINVIDEO)
    set(LFL_WXWIDGETS)
    set(LFL_XTINPUT)
    set(LFL_XTVIDEO)
    set(LFL_X11INPUT)
    set(LFL_X11VIDEO)
    set(LFL_X264)
endmacro(lfl_clear_projects)
lfl_clear_projects()

# lfapp unit tests
add_subdirectory(${CORE_SUBDIR}/lfapp_tests)

# crawler
add_subdirectory(${CORE_SUBDIR}/crawler)

# nlp
add_subdirectory(${CORE_SUBDIR}/nlp)

# speech
add_subdirectory(${CORE_SUBDIR}/speech)
