...
template <class K, class V> struct RedBlackTreeNode {
  ...
  typedef V Value;
  K key;
  unsigned val:31, left=0, right=0, parent, color:1;
  RedBlackTreeNode(K k, unsigned v, unsigned P, bool C=0) : key(k), val(v), parent(P), color(C) {}
...
template <class K, class V, class Node = RedBlackTreeNode<K,V>, class Zipper = RedBlackTreeZipper<Node>,
...
struct RedBlackTree {
  ...
  struct IteratorBase {
    ...
    K key;
    Zipper zipper;
    IteratorBase(int I=0)               : ind(I), key() {}
    IteratorBase(int I, Zipper &&z)     : ind(I), key() { swap(z, zipper); }
    IteratorBase(const IteratorBase &x) : ind(x.ind), key(x.key), zipper(x.zipper) {}
    virtual ~IteratorBase() {}

    virtual void LoadKV() = 0;
    virtual const RedBlackTree *GetParent() const = 0;
    virtual const Node *GetNode() const { return &GetParent()->node[ind-1]; }

    int GetAnchor() const { return ind; }
    int GetIndex()  const { if (auto n = GetNode()) return zipper.GetIndex (*n); return -1; }
    int GetBegKey() const { if (auto n = GetNode()) return zipper.GetBegKey(*n); return -1; }
    int GetEndKey() const { if (auto n = GetNode()) return zipper.GetEndKey(*n); return -1; }

    bool Increment() { CHECK(ind); if ((ind = GetParent()->IncrementNode(ind, &zipper))) LoadKV(); return ind; }
    bool Decrement() { CHECK(ind); if ((ind = GetParent()->DecrementNode(ind, &zipper))) LoadKV(); return ind; }
    void LoadNodeKey(const Node *n) { key = zipper.GetBegKey(*n); }
  };
  ...
  struct Iterator : public IteratorBase {
    ...
    RedBlackTree *tree; 
    V *val=0;
    Iterator(RedBlackTree *T=0, int I=0)         : IteratorBase(I),          tree(T) { if (Base::ind) LoadKV(); }
    Iterator(RedBlackTree *T, int I, Zipper &&z) : IteratorBase(I, move(z)), tree(T) { if (Base::ind) LoadKV(); }
    Iterator& operator--() { if (!Base::Decrement()) val = 0; return *this; }
    Iterator& operator++() { if (!Base::Increment()) val = 0; return *this; }
    bool operator!=(const Iterator &i) { return tree != i.tree || Base::ind != i.ind; }
    const RedBlackTree *GetParent() const { return tree; }
    void LoadKV() { if (auto n = Base::GetNode()) { Base::LoadNodeKey(n); val = &tree->val[n->val]; } }
  };
  ...
  struct ConstIterator : public IteratorBase { 
    ...
    const V *val=0;
    ConstIterator(const RedBlackTree *T=0, int I=0)         : IteratorBase(I),          tree(T) { if (Base::ind) LoadKV(); }
    ConstIterator(const RedBlackTree *T, int I, Zipper &&z) : IteratorBase(I, move(z)), tree(T) { if (Base::ind) LoadKV(); }
    ConstIterator(const Iterator &i) : IteratorBase(i), tree(i.tree), val(i.val) {}
    ConstIterator& operator--() { if (!Base::Decrement()) val = 0; return *this; }
    ConstIterator& operator++() { if (!Base::Increment()) val = 0; return *this; }
    bool operator!=(const ConstIterator &i) { return tree != i.tree || Base::ind != i.ind; }
    const RedBlackTree *GetParent() const { return tree; }
    void LoadKV() { if (auto n = Base::GetNode()) { Base::LoadNodeKey(n); val = &tree->val[n->val]; } }
  };
  ...
  /**/ Iterator  Begin()       {           int n = head ? GetMinNode(head)     : 0; return      Iterator(this, n, Zipper()); }
  ConstIterator  Begin() const {           int n = head ? GetMinNode(head)     : 0; return ConstIterator(this, n, Zipper()); }
  /**/ Iterator RBegin()       { Zipper z; int n = head ? GetMaxNode(head, &z) : 0; return      Iterator(this, n, move(z)); }
  ConstIterator RBegin() const { Zipper z; int n = head ? GetMaxNode(head, &z) : 0; return ConstIterator(this, n, move(z)); }

  K  GetAnchorKey(int id) const { return node[id-1].key; }
  V *GetAnchorVal(int id) { return &val[node[id-1].val]; }
  virtual      Iterator GetAnchorIter(int id)       { return      Iterator(this, id, Zipper()); }
  virtual ConstIterator GetAnchorIter(int id) const { return ConstIterator(this, id, Zipper()); }

  bool          Erase (const K &k)        { Query q(k,1); return EraseNode(&q); }
  /**/ Iterator Insert(const K &k, V &&v) { Query q(k,1); int n=InsertNode(&q, forward<V>(v)); return Iterator(this, n, move(q.z)); }

  ConstIterator Find       (const K &k) const  { Query q(k);       int n=FindNode      (&q); return ConstIterator(this, n, move(q.z)); }
  /**/ Iterator Find       (const K &k)        { Query q(k);       int n=FindNode      (&q); return Iterator     (this, n, move(q.z)); }
  ConstIterator FindIndex  (const K &k) const  { Query q(k,0,1);   int n=FindNode      (&q); return ConstIterator(this, n, move(q.z)); }
  /**/ Iterator FindIndex  (const K &k)        { Query q(k,0,1);   int n=FindNode      (&q); return Iterator     (this, n, move(q.z)); }
  ConstIterator LowerBound (const K &k) const  { Query q(k);       int n=LowerBoundNode(&q); return ConstIterator(this, n, move(q.z)); }
  /**/ Iterator LowerBound (const K &k)        { Query q(k);       int n=LowerBoundNode(&q); return Iterator     (this, n, move(q.z)); }
  ConstIterator SecondBound(const K &k) const  { Query q(k,0,0,1); int n=LowerBoundNode(&q); return ConstIterator(this, n, move(q.z)); }
  /**/ Iterator SecondBound(const K &k)        { Query q(k,0,0,1); int n=LowerBoundNode(&q); return Iterator     (this, n, move(q.z)); }
  ConstIterator LesserBound(const K &k) const  { Query q(k); int n=LowerBoundNode(&q); ConstIterator i(this, n, move(q.z)); if (i.val && k < i.key) --i; return i; }
  /**/ Iterator LesserBound(const K &k)        { Query q(k); int n=LowerBoundNode(&q); Iterator      i(this, n, move(q.z)); if (i.val && k < i.key) --i; return i; }
...
struct RedBlackFingerTree : public RedBlackTree<K, V, Node, Finger> {
  ...
  virtual int ResolveInsertCollision(int ind, typename Base::Query *q, V &&v) { 
    int val_ind = Base::val.Insert(forward<V>(v)), p_ind = ind;
    int new_ind = Base::node.Insert(Node(GetCreateNodeKey(q, &v), val_ind, 0))+1;
    Node *n = &Base::node[ind-1], *nn = &Base::node[new_ind-1];
    if (!n->left) n->left = new_ind;
    else { Finger z; Base::node[(p_ind = Base::GetMaxNode(n->left, &z))-1].right = new_ind; }
    nn->parent = p_ind;
    Base::ComputeAnnotationFromChildrenOnMaxPath(n->left);
    Base::ComputeAnnotationFromChildren(n);
    ComputeAnnotationFromChildrenOnPath(q);
    Base::InsertBalance(new_ind);
    Base::count++;
    return new_ind;
  }

  virtual void ComputeAnnotationFromChildrenOnPath(typename Base::Query *q) {
    for (auto i = q->z.path.rbegin(), e = q->z.path.rend(); i != e; ++i) 
      Base::ComputeAnnotationFromChildren(&Base::node[i->first-1]);
  }

  void LoadFromSortedVal() {
    int n = Base::val.size();
    CHECK_EQ(0, Base::node.size());
    Base::count = Base::val.size();
    Base::head = BuildTreeFromSortedVal(0, n-1, 1, WhichLog2(NextPowerOfTwo(n, true)));
  }

  int BuildTreeFromSortedVal(int beg_val_ind, int end_val_ind, int h, int max_h) {
    if (end_val_ind < beg_val_ind) return 0;
    CHECK_LE(h, max_h);
    int mid_val_ind = (beg_val_ind + end_val_ind) / 2, color = ((h>1 && h == max_h) ? Base::Red : Base::Black);
    int ind = Base::node.Insert(Node(node_value_cb(&Base::val[mid_val_ind]), mid_val_ind, 0, color))+1;
    ...
    Node *n = &Base::node[ind-1];
    n->left  = left_ind;
    n->right = right_ind;
    if (left_ind)  Base::node[ left_ind-1].parent = ind;
    if (right_ind) Base::node[right_ind-1].parent = ind;
    Base::ComputeAnnotationFromChildren(&Base::node[ind-1]);
    return ind;
  }

  V *Update(const K &k, const V &v) {
    typename Base::Query q(k, 1);
    Node *n;
    for (int ind = Base::head; ind; ) {
      n = &Base::node[ind-1];
      if      (q.z.MoreThan(*n, ind, q.key)) ind = q.z.WalkLeft (*n);
      else if (q.z.LessThan(*n, ind, q.key)) ind = q.z.WalkRight(*n);
      else {
...
template <class K, class V> struct RedBlackIntervalTreeNode {
  ...
  unsigned val:31, left=0, right=0, parent, color:1;
  RedBlackIntervalTreeNode(pair<K,K> k, unsigned v, unsigned P, bool C=0) : key(k), val(v), parent(P), color(C) {}
...
struct RedBlackIntervalTree : public RedBlackTree<pair<K, K>, V, Node, Finger> {
  typedef RedBlackTree<pair<K, K>, V, Node, Finger> Base;

  const V *IntersectOne(const K &k) const { int n=IntersectOneNode(k); return n ? &Base::val[Base::data[n-1].val] : 0; }
  /**/  V *IntersectOne(const K &k)       { int n=IntersectOneNode(k); return n ? &Base::val[Base::data[n-1].val] : 0; }

  typename Base::ConstIterator IntersectAll(const K &k) const { Finger z(k, Base::head); int n=IntersectAllNode(&z); return typename Base::ConstIterator(this, n, move(z)); }
  typename Base::Iterator      IntersectAll(const K &k)       { Finger z(k, Base::head); int n=IntersectAllNode(&z); return typename Base::Iterator     (this, n, move(z)); }
  void IntersectAllNext(typename Base::ConstIterator *i) const { if ((i->ind = IntersectAllNode(&i->zipper))) i->LoadKV(); else i->val=0; }
  void IntersectAllNext(typename Base::Iterator      *i)       { if ((i->ind = IntersectAllNode(&i->zipper))) i->LoadKV(); else i->val=0; }
...
template <class K, class V> struct PrefixSumKeyedRedBlackTreeNode {
  ...
  unsigned val:31, left=0, right=0, parent, color:1;
  PrefixSumKeyedRedBlackTreeNode(K k, unsigned v, unsigned P, bool C=0) : key(k), val(v), parent(P), color(C) {}
...
struct PrefixSumKeyedRedBlackTree : public RedBlackFingerTree<K, V, Node, Finger> {
  ...
  int Total() const {
    auto i = Base::RBegin();
    if (i.ind) i.zipper.WalkRight(Base::node[i.ind-1]);
    return i.zipper.sum;
  }
