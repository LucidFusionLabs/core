...
#ifndef __LFL_LFAPP_DOM_H__
#define __LFL_LFAPP_DOM_H__
namespace LFL {
namespace DOM {
    struct Node;
    struct Attr;
...
namespace LFL_STL_HASH_NAMESPACE {
    template<> struct hash<LFL::DOM::DOMString> { size_t operator()(const LFL::DOM::DOMString &x) const {
        return x.Hash(); 
    } };
};
namespace LFL {
namespace DOM {
    ...
    template <class X> struct List : public Object {
        ...
        X item(int index) { return (index >= 0 && index < data.size()) ? data[index] : 0; }
...
#       include "crawler/html_elements.h"
    ...
    template <class X> struct NamedMap : public Object {
        ...
        X getNamedItem(const DOMString &name) {
        ...
        X item(int index) { return (index >= 0 && index < data_val.size()) ? data_val[index] : 0; }
...
#define DERIVE_NODE(name, type) DERIVE_NODE_FROM(name, Node, type)
    ...
    struct Document : public Node { DERIVE_NODE_IMPL(Document, DOCUMENT_NODE);
        ...
        Document(Browser *b, Allocator *a, GUI *g) : Node(DOCUMENT_NODE, this), doctype(this), ownerBrowser(b), alloc(a), gui(g), style_context(0) {}
...
#define PARSE_ATTR_END }
    ...
    struct HTMLFormElement : public HTMLElement { DERIVE_ELEMENT(HTMLFormElement, HTML_FORM_ELEMENT) {}
        ...
        long length() { return elements.length(); }
        void submit();
        void reset();
    };
    struct HTMLSelectElement : public HTMLElement { DERIVE_ELEMENT(HTMLSelectElement, HTML_SELECT_ELEMENT), form(0) {}
        ...
        long length() { return options.length(); }        
...
#define HTMLTableIter(t) HTMLTableRowIter(t) HTMLTableColIter(row)
    ...
    struct HTMLDocument : public Document {
        HTMLDocument(Browser *b, Allocator *a, GUI *g) : Document(b, a, g) {}
